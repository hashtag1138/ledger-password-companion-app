...
class FakeLedgerTransport(
    ...
    initialMetadatas: ByteArray = ByteArray(storageSize) { 0x00.toByte() },
) : LedgerTransport {
    ...
    private val loadBuffer: ByteArray = ByteArray(storageSize) { 0x00.toByte() }
    ...
    private fun appInfo(): ApduResponse {
        ...
        val payload = byteArrayOf(0x01.toByte(), name.size.toByte()) + name + byteArrayOf(version.size.toByte()) + version
