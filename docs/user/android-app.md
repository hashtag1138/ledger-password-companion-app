...
- `Info` stays local to the app and exported `backup.json` files, and is never sent to the Ledger;
- `Info` is stored in clear text;
...
- `Export backup.json` writes the local state to a file selected through the Android picker, with a suggested timestamped name such as `ledger-passwords-backup-2026-06-04_14-37-12.json`.
